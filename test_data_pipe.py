...
def save_dataloader_cache(cache_key, train_loader, test_loader, train_sampler, test_sampler, token_dict, cache_dir):
...
def load_dataloader_cache(cache_key, config, cache_dir):
    ...
    try:
        ...
        return train_loader, test_loader, train_sampler, test_sampler, cache_data['token_dict']
...
def main():
    ...
    try:
        ...
        if cached_result is not None:
            train_loader, test_loader, train_sampler, test_sampler, token_dict = cached_result
            ...
            # Save to cache
            print("\nSaving cache...")
            save_dataloader_cache(cache_key, train_loader, test_loader, train_sampler, test_sampler, token_dict, cache_dir)
        ...
        if token_dict:
            ...
            for i, (token_id, count) in enumerate(token_dict.most_common(10)):
                try:
                    token_text = tokenizer.decode([token_id]) if hasattr(tokenizer, 'decode') else str(token_id)
