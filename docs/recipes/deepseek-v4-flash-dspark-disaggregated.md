...
- SGLang `v0.5.14` patched with the checked-in capture patch, which includes
  DeepSeek-V4's mHC-aware DSpark capture hook
...
CUDA_VISIBLE_DEVICES=4,5,6,7 specforge train \
  -c examples/configs/deepseek-v4-flash-dspark-disaggregated.yaml
```
...
```bash
specforge train -c examples/configs/deepseek-v4-flash-dspark-disaggregated.yaml \
