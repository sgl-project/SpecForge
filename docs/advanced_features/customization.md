...
| DSpark | `eager`, `sdpa`, `flex_attention`, offline/online Ulysses-only `usp` |
...
DSpark offline and server-streaming online use the same switch for Ulysses
sequence parallelism, but currently require `training.sp_ring_size: 1`.
