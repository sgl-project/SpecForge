...
for ax, (dataset, data) in zip(flat, DATA.items()):
    ...
    ax.set_title(
        dataset, color=INK, fontsize=10.5, fontweight="bold", pad=10, loc="left"
    )
...
if SETUP:
    title = f"{title} · {SETUP}"
fig.suptitle(
    title, x=0.045, y=1.0, ha="left", fontsize=12.5, color=INK, fontweight="bold"
)

fig.subplots_adjust(
    left=0.065, right=0.995, top=0.92, bottom=0.075, wspace=0.12, hspace=0.32
)
fig.savefig(
    "/data/projects/speculative/SpecForge/blog/dspark-speedup.svg",
    facecolor=SURFACE,
    bbox_inches="tight",
)
fig.savefig(
    "/tmp/claude-0/-data-projects-speculative-SpecForge/35941c0a-904a-4675-a8c7-4219521ba5d9/scratchpad/dspark-speedup.png",
    facecolor=SURFACE,
    dpi=140,
    bbox_inches="tight",
)
