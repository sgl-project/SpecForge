...
def load_and_prepare_data(
    ...
    if data_files:
        ...
        datasets_to_combine = [dataset[split] for split in dataset_splits if split in dataset]
        combined_data = concatenate_datasets(datasets_to_combine) if len(datasets_to_combine) > 1 else datasets_to_combine[0]
    else:
    ...
    if end == -1:
        ...
        print(f"Processing all data from index {start} to end (total: {total_length} samples)")
    else:
        actual_end = min(end, total_length)
        print(f"Processing data from index {start} to {actual_end} (total: {total_length} samples)")
...
def extract_conversations(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not messages:
        if 'prompt' in data and 'response' in data:
            return {"system_prompt": None, "queries": [data['prompt']], "responses": [data['response']]}
        else:
...
def process_conversations(raw_data: List[Dict]) -> tuple[List[Dict], List[Dict]]:
    ...
    for i, data in enumerate(raw_data):
        ...
        processed_conversations.append({
            ...
            "conversations": [],
            "system_prompt": conv["system_prompt"] # ### NEW ### Store system prompt
        })
        ...
        if conv["system_prompt"]:
            conversation_history.append({"role": "system", "content": conv["system_prompt"]})
        ...
        for j, query in enumerate(conv["queries"]):
            ...
            arguments.append({
                ...
                "original_response": conv["responses"][j] if j < len(conv["responses"]) else "",
                "current_query": query
            })
            
            # Add assistant response to history for next round (if available)
            if j < len(conv["responses"]):
                conversation_history.append({"role": "assistant", "content": conv["responses"][j]})
...
def main(args):
    ...
    print(f"Generated {len(arguments)} responses for {len(processed_conversations)} conversations")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Dataset processing and generation with SGLang')
    
    # Data processing arguments
    parser.add_argument('--start', type=int, default=0, help='Start index')
    parser.add_argument('--end', type=int, default=-1, help='End index (-1 means process all data)')
    ...
    parser.add_argument('--dataset-name', type=str, default="HuggingFaceH4/ultrachat_200k", help='Dataset name or path')
    parser.add_argument('--dataset-path', type=str, default=None, help='Local dataset path')
    parser.add_argument('--data-files', type=str, default=None, help='Data files path for JSON datasets')
    parser.add_argument('--dataset-split', type=str, nargs='+', default=["train_sft"], help='Dataset splits to use')
    parser.add_argument('--seed', type=int, default=42, help='Random seed for shuffling')
    parser.add_argument('--max-query-len', type=int, default=4000, help='Max query length')
