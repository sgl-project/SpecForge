...
class OnlineDSparkModel(OnlineDFlashModel):
    ...
    def _compute_dspark_loss(
        ...
        *,
        ...
        # Device-side asserts avoid draining the stream after a collective.
        if self.loss_type == "dpard":
            torch._assert_async(
                (global_dpard_position_den > 0).any(),
                "D-PARD actor has no valid position",
            )
            loss = world_size * dpard_loss_num / global_dpard_position_den
            if self.dspark_confidence_head_alpha > 0:
                torch._assert_async(
                    (global_dpard_confidence_den > 0).any(),
                    "D-PARD confidence has no valid block",
                )
                loss = loss + (
                    world_size
