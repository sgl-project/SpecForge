...
def initialize_tree(input_ids, pixel_values, model, past_key_values, logits_processor):
    outputs, orig, hidden_states, past_key_values = model(
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.draft_model.topK_genrate(hidden_states, input_ids, logits_processor)
...
def reset_tree_mode(
    ...
    model.target_model.model.language_model.tree_mask = None
    model.target_model.model.language_model.tree_mode = None
...
def tree_decoding(
    ...
    outputs, tree_logits, hidden_states,past_key_values = model(
        tree_candidates,
        output_orig=True,
...
def update_inference_inputs(
    ...
    for past_key_values_data in cache.past_key_values_data_list:
        tgt = past_key_values_data[..., select_indices.to(past_key_values_data.device), :]
        dst = past_key_values_data[..., prev_input_len: prev_input_len + tgt.shape[-2], :]
        dst.copy_(tgt, non_blocking=True)
        # add
        dst_end = prev_input_len + tgt.shape[-2]  # 606
        if dst_end < past_key_values_data.shape[-2]:
            past_key_values_data[..., dst_end:, :].zero_()
    # Update the current length tensor
    cache.current_length_data.fill_(prev_input_len + tgt.shape[-2])
    cache.sync_from_past_data()
    # print(cache.current_length_data[0])
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.draft_model.topK_genrate(accept_hidden_state_new,
                                              input_ids=torch.cat((input_ids, token.to(input_ids.device)), dim=1),
