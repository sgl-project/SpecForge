...
def _get_text_config_for_embeddings(config):
    """
    For VLM configs (Qwen3_5, Qwen3_VL, etc.), vocab_size/hidden_size are in text_config.
...
class TargetEmbeddingsAndHead(nn.Module):
    ...
    Supports VLM configs (Qwen3.5, Qwen3-VL) where vocab_size/hidden_size live in text_config.
    """

    def __init__(self, config):
        super().__init__()
        self.config = config
        text_cfg = _get_text_config_for_embeddings(config)

        self.embed_tokens = nn.Embedding(
            text_cfg.vocab_size,
            text_cfg.hidden_size,
            padding_idx=getattr(text_cfg, "pad_token_id", None),
        )
