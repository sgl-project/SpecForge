...
def _patch_vlm_model_for_layer_capture(model: nn.Module, layer_ids: List[int]):
    """
    Monkey-patch SGLang VLM models that lack native set_eagle3_layers_to_capture support.
    This patches the inner language model to capture specified layers' hidden states,
    and the outer ConditionalGeneration model to unpack the tuple and pass aux_hidden_states
    to the logits_processor.

    Supports: Qwen3VLForConditionalGeneration, Qwen3_5(Moe)ForConditionalGeneration,
    and their subclasses (e.g. Qwen3VLMoeForConditionalGeneration).
    """
    inner_model = getattr(model, "model", None)
    if inner_model is None:
        return False

    if not hasattr(inner_model, "layers_to_capture"):
        inner_model.layers_to_capture = []

    inner_model.layers_to_capture = [val + 1 for val in layer_ids]

    if not hasattr(model, "capture_aux_hidden_states"):
    ...
    if not hasattr(model, "_layer_capture_patched"):
        original_forward_method = model.forward

        def _make_patched_forward(orig_fwd):
            import functools
            import types

            if isinstance(orig_fwd, types.MethodType):
                orig_func = orig_fwd.__func__
            else:
                orig_func = orig_fwd

            @functools.wraps(orig_func)
            def patched_forward(self_model, input_ids, positions, forward_batch,
                                get_embedding=False, pp_proxy_tensors=None, **extra_kwargs):
                if hasattr(forward_batch, 'mrope_positions') and hasattr(self_model, 'is_mrope_enabled') and self_model.is_mrope_enabled:
                ...
                hidden_states = general_mm_embed_routine(
                    ...
                    language_model=self_model.model,
                    multimodal_model=self_model,
                    positions=positions,
                    use_deepstack=getattr(self_model, 'use_deepstack', False),
                    pp_proxy_tensors=pp_proxy_tensors,
                )

                aux_hidden_states = None
                if self_model.capture_aux_hidden_states and isinstance(hidden_states, tuple):
                    hidden_states, aux_hidden_states = hidden_states

                if self_model.pp_group.is_last_rank:
                    if not get_embedding:
                        return self_model.logits_processor(
                            input_ids, hidden_states, self_model.lm_head,
                            forward_batch, aux_hidden_states,
                        )
                    else:
                        return self_model.pooler(hidden_states, forward_batch)
                else:
        ...
        import types
        model.forward = types.MethodType(_make_patched_forward(original_forward_method), model)
        model._layer_capture_patched = True
...
class SGLangDFlashTargetModel(DFlashTargetModel):
    ...
    def set_capture_layers(self, layer_ids: List[int]) -> None:
        ...
        if hasattr(self.model_runner.model, "set_eagle3_layers_to_capture"):
            ...
            _patch_vlm_model_for_layer_capture(self.model_runner.model, layer_ids)
...
class HFDFlashTargetModel(DFlashTargetModel):
        ...
        **kwargs,
        ...
        if model_type in QWEN3_VL_MODEL_TYPES:
            ...
            if model_type == "qwen3_vl":
                ...
                # Qwen3.5/3.6: try transformers 5.x, then AutoModel fallback
                try:
                ...
                if model_cls is None:
                    raise ImportError(
                        f"model_type '{model_type}' requires transformers >= 5.0 for HF backend. "
                        "Use --target-model-backend sglang instead, or upgrade transformers."
                    )
    ...
    def generate_dflash_data(
        ...
        if self.model_type in QWEN3_VL_MODEL_TYPES:
            ...
            if hasattr(self.model, "config") and hasattr(self.model.config, "image_token_id"):
                ...
                target_kwargs["mm_token_type_ids"] = mm_token_type_ids
        ...
        if self.model_type in QWEN3_VL_MODEL_TYPES:
            ...
            if target_inner_model is not None and hasattr(
                ...
                if "mm_token_type_ids" in target_kwargs:
                    rope_kwargs["mm_token_type_ids"] = target_kwargs["mm_token_type_ids"]
