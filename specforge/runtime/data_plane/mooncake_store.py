...
class MooncakeFeatureStore(FeatureStore):
    ...
    *tensor*, keyed ``{store_id}/{sample_id}/g{gen}/{name}``. ``put()`` writes each
    ...
    def get_returns_fresh_tensors(self) -> bool:
        # Raw reads allocate fresh storage from FeatureSpec.
        return True
    ...
    def _store_get_tensor(self, key: str, out: torch.Tensor) -> None:
        ...
        # torch.empty'd buffer as uninitialized garbage. Reject it rather than
        # hand the trainer silently-corrupt data (B5: never serve wrong bytes).
        if int(rc) != nb:
            raise KeyError(
        ...
        *,
        ...
        try:
            ...
            leaked = [
                ...
                if not self._store_remove(self._tkey(sample_id, gen, name), force=False)
            ]
    ...
    def get(
        ...
        *,
        ...
        out, gen = self._get_tensors(sample_ref, wanted, pin_memory=pin_memory)
        if str(device) != "cpu":
    ...
    def _try_physical_free(
        ...
        *,
        ...
        One object backs each tensor, so remove every per-tensor key of the
        sample's current generation.
