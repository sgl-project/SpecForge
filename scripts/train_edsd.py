...
def build_parser() -> ArgumentParser:
    ...
    training_group.add_argument(
        ...
        default=0.02,
        help=(
            "EDSD mask_p: per-epoch curriculum drop ratio is "
            "min(max((num_epochs - epoch_idx) * drop_ratio_scale, 0.0), 0.4). "
            "Largest at epoch 0, decays to ~0 by the final epoch."
        ),
    )
    training_group.add_argument(
        "--step-n-schedule",
        ...
        default=None,
        help=(
            "EDSD step_n: explicit per-epoch TTT length list (monotonic "
            "non-decreasing, each in [1, --ttt-length]). epoch >= len uses the "
            "last value. If unset, the length is auto-scheduled by EDSD Eq. 6 "
...
def build_target_model(
...
) -> Tuple[Union[Eagle3TargetModel, TargetHead], None]:
    """
    ...
    if is_online:
        ...
        elif hasattr(draft_model_config, "target_layer_ids"):
            target_model.set_aux_hidden_states_layers(
                draft_model_config.target_layer_ids
            )
        else:
            target_model.set_aux_hidden_states_layers()
...
def sanity_check(args: Namespace) -> None:
    ...
    Returns:
        None
    """
    if args.step_n_schedule is not None:
        if len(args.step_n_schedule) == 0:
            raise ValueError("--step-n-schedule must not be empty")
        prev = 0
        for i, v in enumerate(args.step_n_schedule):
            if not isinstance(v, int) or v < 1:
                raise ValueError(
                    f"--step-n-schedule values must be positive ints, got {v} at index {i}"
                )
            if v < prev:
                raise ValueError(
                    f"--step-n-schedule must be monotonic non-decreasing, "
                    f"got {v} < {prev} at index {i}"
                )
            prev = v
        if args.step_n_schedule[-1] > args.ttt_length:
            raise ValueError(
                f"--step-n-schedule values must be <= --ttt-length "
                f"({args.ttt_length}), got max {args.step_n_schedule[-1]}"
            )
...
def build_draft_model(args: Namespace) -> Tuple[AutoDraftModelConfig, nn.Module]:
    ...
    # EDSD uses its own draft model class (EdsdDraftModel). We construct it
    # directly rather than via AutoEagle3DraftModel, whose single config->class
    # mapping is already occupied by LlamaForCausalLMEagle3.
    # Tag the config so the saved config.json advertises EdsdDraftModel (useful
    # for serving / re-loading), regardless of how it was generated.
    draft_model_config.architectures = ["EdsdDraftModel"]
    if draft_model_last_checkpoint:
        draft_model = EdsdDraftModel.from_pretrained(
            draft_model_last_checkpoint,
            attention_backend=args.attention_backend,
            torch_dtype=torch.bfloat16,
        ).cuda()
    else:
        draft_model = EdsdDraftModel(
            draft_model_config,
            attention_backend=args.attention_backend,
        )
        draft_model = draft_model.to(dtype=torch.bfloat16).cuda()
...
def run_forward(
    ...
    if is_online:
        # generate eagle3 target data from the target model (text-only)
...
def record_metrics(
    ...
    if acc_corrects is not None and acc_denoms is not None:
        ...
        assert corrects.shape[0] >= 1, "expected at least one TTT position"
        ...
        assert accuracies.shape[0] >= 1, "expected at least one TTT position"
    ...
    assert acceptance_rates.shape[0] >= 1, "expected at least one TTT position"
...
def main():
    ...
    target_model, _ = build_target_model(args, draft_model_config, is_online)
    ...
    if is_online:
        eagle3_model = OnlineEdsdModel(
            ...
            drop_ratio_scale=args.drop_ratio_scale,
            step_n_schedule=args.step_n_schedule,
        )
    else:
        # offline: the target_model is TargetHead, not a full model
        eagle3_model = OnlineEdsdModel(
            draft_model=draft_model,
            length=args.ttt_length,
            attention_backend=args.attention_backend,
    ...
    # ================================================
    # Worst-case memory probe before training begins.
    # Forces actual_length = --ttt-length (the final-epoch curriculum value),
    # which is the maximum any real training step will ever reach. If this
    # single forward+backward fits in memory, no subsequent epoch can OOM on
    # the actual_length axis. If it OOMs here, the user can shrink batch/seq
    # before wasting hours of training. Probe grads are discarded (no
    # optimizer.step), so the real first step starts clean.
    # ================================================
    probe_iter = iter(train_dataloader)
    try:
        probe_data = next(probe_iter)
    ...
    if probe_data is not None:
        draft_model.train()
        print_on_rank0("Full memory probe at max TTT length...")
        try:
            (
                probe_plosses,
                *_,
                ...
                target_model,
                is_online,
                epoch=args.num_epochs - 1,  # force actual_length = S_max
                total_epochs=args.num_epochs,
            )
            probe_loss = sum(probe_plosses) / len(probe_plosses)
            probe_loss.backward()
        except torch.OutOfMemoryError as e:
            torch.cuda.empty_cache()
            raise torch.OutOfMemoryError(
                "Worst-case memory probe failed at max TTT length. "
                "Reduce batch size or sequence length before launching training."
            ) from e
        finally:
            # Drop the probe's gradients so the real first step starts clean.
            # BF16Optimizer only clears grads inside .step(); the probe must not
            # rely on it, and must never call .step().
            with torch.no_grad():
                for p in optimizer.model_params:
                    p.grad = None
            del probe_plosses, probe_loss
            torch.cuda.empty_cache()
        print_on_rank0("Memory probe passed. Starting training.")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step_in_epoch, data in enumerate(progress_bar):
            ...
            with torch.no_grad():
                # EDSD's TTT length can change across an epoch boundary within a
                # single logging interval; reset interval accumulators if the
                # current step's length no longer matches the accumulated one.
                if (
                    metric_correct_sums is not None
                    and len(metric_correct_sums) != len(acc_corrects)
                ):
                    metric_correct_sums = None
                    metric_denom_sums = None
                    metric_loss_weighted_sums = None
            ...
            if (
                ...
                # compute average over all minibatches (only positions that
                # actually produced outputs, in case the eval TTT length varies)
                eval_acces = [torch.stack(acc).mean() for acc in eval_acces if len(acc) > 0]
                eval_acceptance_rates = [
                    torch.stack(ar).mean() for ar in eval_acceptance_rates if len(ar) > 0
                ]
                eval_plosses = [torch.stack(pl).mean() for pl in eval_plosses if len(pl) > 0]
