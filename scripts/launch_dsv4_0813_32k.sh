...
servers)
  ...
  # Three sink clients per capture server: each Mooncake client serves remote
  # reads on one TCP worker thread; striping puts across clients multiplies
  # the sink's serve bandwidth for the trainer's feature fetches.
