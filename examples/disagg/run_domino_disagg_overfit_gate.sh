#!/bin/bash
# Strict model-parameterized, one-sample Domino disaggregated overfit gate:
...
export DISAGG_STORE_ID=${DISAGG_STORE_ID:-domino-overfit-$RUN_TAG}
...
"$PYTHON" "$ROOT_DIR/scripts/gates/select_overfit_sample.py" \
...
wait "$PRODUCER_PID"
"$PYTHON" "$ROOT_DIR/scripts/gates/check_overfit_metrics.py" \
    --log-path "$TRAIN_LOG" \
...
if [[ "${RUN_REAL_SERVING_GATE:-0}" == "1" ]]; then
    ...
    SERVING_CONTEXT_LENGTH="$MAX_LENGTH" \
    WORK_DIR="$WORK_DIR/real_serving" \
        bash "$ROOT_DIR/examples/disagg/run_domino_dflash_serving_gate.sh"
else
    echo "Real serving is a separate strict gate. Run it with:"
    echo "CHECKPOINT_PATH=$LATEST_CHECKPOINT \\"
    echo "  PROMPT_ARTIFACT_PATH=$PROMPT_ARTIFACT_PATH \\"
    echo "  bash examples/disagg/run_domino_dflash_serving_gate.sh"
fi

echo "DOMINO-DISAGG-OVERFIT-PASSED: $WORK_DIR"
