...
: "${DRAFT_CONFIG_PATH:?set DRAFT_CONFIG_PATH to the matching Domino config}"
: "${EMBEDDING_KEY:?set EMBEDDING_KEY to the target embedding tensor key}"
SERVED_MODEL=${SERVED_MODEL:-domino-target}
...
WORK_DIR=${WORK_DIR:-$ROOT_DIR/outputs/domino-real-serving-$(date +%Y%m%dT%H%M%S)}
...
PYTHONPATH="$ROOT_DIR${PYTHONPATH:+:$PYTHONPATH}" \
    "$SGLANG_PYTHON" "$ROOT_DIR/scripts/gates/run_dflash_chat_serving_gate.py" \
...
echo "DOMINO-REAL-DFLASH-SERVING-PASSED: $RESULT_PATH"
