#!/bin/bash
# DSpark online training for Qwen3.5-4B on Ascend NPU.
#
...
# The L1 / confidence losses need the target model's FINAL hidden state; the HF
# backend (used below) always surfaces it on NPU.
#
...
torchrun \
    ...
    --draft-config-path "$ROOT_DIR/configs/qwen3.5-4b-dspark.json" \
    --train-data-path "$TRAIN_DATA_PATH" \
    --output-dir "$ROOT_DIR/outputs/qwen3.5-4b-dspark-npu" \
    --num-epochs 10 \
    --batch-size 1 \
