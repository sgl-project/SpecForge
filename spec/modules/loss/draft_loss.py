...
class DraftLoss(nn.Module, SFTLoss):
    ...
    def compute_draft_loss(
        ...
        if isinstance(backbone_output_hidden_states, DTensor):
            ...
            local_backbone_output_hidden_states = (
                backbone_output_hidden_states.to_local()[loss_mask]
            )
            backbone_output_hidden_states = DTensor.from_local(
                local_backbone_output_hidden_states, mesh, placements
            )  # [num_valid, embed_dim]
        else:
            backbone_output_hidden_states = backbone_output_hidden_states[
                loss_mask
            ]  # [num_valid, embed_dim]
        ...
        if isinstance(draft_output_hidden_states, DTensor):
            ...
            local_draft_output_hidden_states = draft_output_hidden_states.to_local()[
                loss_mask
            ]
            ...
            draft_output_hidden_states = draft_output_hidden_states[
                loss_mask
            ]  # [num_valid, embed_dim]
        ...
        with torch.no_grad():
            backbone_head_out = self.linear_projection(
            ...
            backbone_probs = nn.Softmax(dim=-1)(
                backbone_head_out
            ).detach()  # [num_valid, vocab_size]
            _, target = torch.max(backbone_head_out, 1)

        # draft model logic
        draft_head_out = self.linear_projection(
        ...
        loss_reg = torch.sum(torch.mean(loss_reg, 1)) / (loss_mask.sum() + 1e-5)
