...
class DraftMultiHeadAttention(nn.Module):
    """
    Using KVCache in Training
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.ttt_kvcache = TrainableKVCache()
    ...
    def forward(
        ...
        *,
        ...
        # update kv cache
        full_k, full_v = self.ttt_kvcache(k, v, past_k, past_v)
        ...
        if self.num_heads != self.num_kv_heads:
            expand_shape = (b, self.num_kv_heads, q_per_kv, -1, self.head_dim)
            k = full_k.unsqueeze(2).expand(expand_shape).flatten(1, 2)
            v = full_v.unsqueeze(2).expand(expand_shape).flatten(1, 2)
        ...
        # reshape the output to be the same shape as the input
        output = output.transpose(1, 2).contiguous().view(b, s_x, -1)
        return self.output_proj(output), full_k, full_v
