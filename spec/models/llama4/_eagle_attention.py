...
def create_ttt_mask(mask, bsz: int, seq_len: int, ttt_step: int):
    ...
    block_mask = create_block_mask(combined_mask_fn, bsz, None, seq_len, total_kv_len)
...
if __name__ == "__main__":
    ...
    print("--- Testing Flex Attention Mask (Corrected) ---")
    print(
        f"Parameters: bsz={bsz_test}, seq_len(k)={seq_len_test}, ttt_step={ttt_step_test} (total blocks i=3)\n"
    )

    # Create the mask object
    ttt_mask_object = create_ttt_mask(
        bsz=bsz_test, seq_len=seq_len_test, ttt_step=ttt_step_test
    )
    ...
    print(
        "To verify its correctness, we can 'materialize' it into a dense tensor for inspection.\n"
    )
