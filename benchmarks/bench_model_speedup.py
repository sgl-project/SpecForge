...
CUDA_VISIBLE_DEVICES=0,1,2,3 python3 bench_model_speedup.py \
    ...
    --benchmark-list mtbench:80 gsm8k:200 \
...
def parse_args():
    ...
    parser.add_argument("--benchmark-list", type=str, nargs="+", default=["mtbench:80", "gsm8k:200", "humaneval:200"])
...
def get_mtbench_conversations(num_prompts: int, split_category: bool = True):
    ...
    for q in questions:
        ...
        sub_bench_name = f"{bench_name}-{q['category']}" if split_category else bench_name
...
def get_gsm8k_conversations(num_prompts: int, num_shots: int = 5):
    ...
    for i in range(len(questions)):
        conversation = []
        conversation.append({"role": "user", "content": few_shot_examples + get_one_example(questions, i, False)})
        bench_conversations[bench_name].append(conversation)
    return bench_conversations 

def get_humaneval_conversations(num_prompts: int):
    questions = load_dataset("openai/openai_humaneval")["test"]
...
def launch_sglang_server(server_args: ServerArgs, base_url: str, batch_size: int, steps: int, topk: int, num_draft_tokens: int):
    sglang_args = []
    if steps > 0:
        sglang_args.extend([
...
async def build_conversation(req_obj: RequestFuncObject, client: AsyncOpenAI, pbar: Optional[tqdm] = None) -> str:
    messages = []
...
async def limited_build_conversation(req_obj: RequestFuncObject, client: AsyncOpenAI, semaphore: Optional[asyncio.Semaphore] = None, pbar: Optional[tqdm] = None):
    if semaphore is None:
        return await build_conversation(req_obj=req_obj, client=client, pbar=pbar)
    async with semaphore:
        return await build_conversation(req_obj=req_obj, client=client, pbar=pbar)

async def run_benchmark(conversation_list, server_args: ServerArgs, client: AsyncOpenAI, semaphore: Optional[asyncio.Semaphore] = None):
    ...
    for i, conversation in enumerate(conversation_list):
        ...
        tasks.append(
            asyncio.create_task(limited_build_conversation(req_obj=req_obj, client=client, semaphore=semaphore, pbar=pbar))
        )
...
def main():
    ...
    if len(bench_conversations) == 0:
        print("no prompt is set, please check whether --benchmark-list is set correctly!")
        exit()    
    ...
    for batch_size, steps, topk, num_draft_tokens in configs:
        ...
        for bench_name, conversation_list in bench_conversations.items():
            ...
            outputs = asyncio.run(run_benchmark(conversation_list, server_args, client, semaphore))
