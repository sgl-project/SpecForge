...
Usage:
    ...
    2. Run this script on a separate GPU:
       CUDA_VISIBLE_DEVICES=1 python tests/test_modeling/test_target/test_remote_backend/compare_local_remote_inference.py
"""
...
def compare_tensors(
    name: str, local: torch.Tensor, remote: torch.Tensor, atol: float, rtol: float
) -> bool:
    ...
    if local_shape != remote_shape:
        ...
        if local_squeezed.shape == remote_squeezed.shape:
            logger.warning(
                f"  {name}: Shape differs but squeezed shapes match - local {local_shape} vs remote {remote_shape}"
            )
            local = local_squeezed
            remote = remote_squeezed
        else:
            logger.error(
                f"  {name}: Shape mismatch - local {local_shape} vs remote {remote_shape}"
            )
            return False

    if local.dtype != remote.dtype:
        logger.info(
            f"  {name}: dtype differs - local {local.dtype} vs remote {remote.dtype} (converting for comparison)"
        )
    ...
    if is_close:
        logger.info(
            f"  {name}: MATCH (max_diff={max_diff:.6e}, mean_diff={mean_diff:.6e})"
        )
    else:
        logger.error(
            f"  {name}: MISMATCH (max_diff={max_diff:.6e}, mean_diff={mean_diff:.6e})"
        )
...
def main():
    ...
    remote_model = RemoteEagle3TargetModel(
        config=remote_config, aux_hidden_states_layers=[1, -1, -4]
    )
