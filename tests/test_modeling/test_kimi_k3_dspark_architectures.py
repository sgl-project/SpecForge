...
class TestKimiK3DSparkArchitectures(unittest.TestCase):
    def test_production_configs_match_k3_target(self):
        cases = [
        ...
        for filename, architecture in cases:
            with self.subTest(filename=filename, architecture=architecture):
                self._assert_production_config(filename, architecture)

    def _assert_production_config(self, filename, architecture):
    ...
    def test_reference_full_attention_config_is_exact_old_architecture(self):
        config = json.loads(
    ...
    def test_reference_full_attention_recipe_preserves_exact_old_contract(self):
    ...
    def test_training_recipes_preserve_reference_run_contract(self):
        for filename in [
    ...
    def _assert_training_recipe(self, filename):
        ...
        assert "kimi-k3-openperfectblend-regen-439c2fdc" in (
    ...
    def test_tiny_5mla_forward_and_backward(self):
    ...
    def test_mla_absorbed_and_expanded_attention_are_algebraically_equivalent(self):
        torch.manual_seed(7)
    ...
    def test_tiny_4kda_1mla_forward_and_backward(self):
    ...
    def test_fla_kda_splits_independent_blocks_below_cuda_grid_z_limit(self):
        calls = []
        ...
        with (
            patch.object(kimi_k3_dspark, "_CUDA_MAX_GRID_DIM_Z", 8),
            patch.object(
                kimi_k3_dspark,
                "_load_fla_chunk_kda",
    ...
    def test_kda_resets_state_between_proposal_blocks(self):
    ...
    def test_hybrid_rejects_noncanonical_layer_order(self):
        config = _tiny_config("KimiK3DSpark4KDA1MLADraftModel")
        config.draft_layer_types = ["mla", "kda", "kda", "kda", "kda"]
        with self.assertRaisesRegex(ValueError, "layer pattern"):
            KimiK3DSpark4KDA1MLADraftModel(config)

    def test_5mla_rejects_noncanonical_layer_count(self):
        config = _tiny_config("KimiK3DSpark5MLADraftModel", layers=4)
        with self.assertRaisesRegex(ValueError, "exactly 5 layers"):
            KimiK3DSpark5MLADraftModel(config)

    def test_mla_requires_k3_attention_features(self):
        for field in ("mla_use_nope", "mla_use_output_gate"):
            with self.subTest(field=field):
                self._assert_mla_requires_k3_attention_feature(field)

    def _assert_mla_requires_k3_attention_feature(self, field):
