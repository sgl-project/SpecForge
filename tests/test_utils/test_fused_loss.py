...
# triton-ascend's driver (NPUDriver.get_current_device) is not Dynamo-traceable,
# so torch.compile(fullgraph=True) cannot wrap any fused kernel on Ascend NPU
# (platform limitation, pre-existing for every loss). CUDA covers fullgraph.
skip_fullgraph_on_npu = unittest.skipIf(
    DEVICE == "npu",
    "triton-ascend driver is not Dynamo-traceable (fullgraph unsupported on NPU)",
)
