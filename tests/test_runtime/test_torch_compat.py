"""TorchInductor defaults installed by ``specforge.torch_compat``."""
...
class FlexAttentionInductorTest(unittest.TestCase):
    def test_flex_attention_pins_aten_and_triton_candidates(self):
        with (
            mock.patch.dict(os.environ, {}, clear=True),
            mock.patch.object(inductor_config, "max_autotune_gemm_backends", "TRITON"),
        ):
            ...
            self.assertEqual(inductor_config.max_autotune_gemm_backends, "ATEN,TRITON")
    ...
    def test_other_attention_backends_leave_inductor_alone(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            self.assertFalse(configure_flex_attention_inductor("sdpa"))
            self.assertNotIn("TORCHINDUCTOR_MAX_AUTOTUNE_GEMM_BACKENDS", os.environ)
