...
def test_preprocess_vlm_conversations():
    ...
    # for i in range(input_ids.shape[-1]):
        if current_mask == loss_mask[i]:
            ...
            decoded_text = processor.tokenizer.decode(current_ids, skip_special_tokens=False)
            if current_mask == 0:
    ...
    print(f"{GREEN}{processor.tokenizer.decode(current_ids, skip_special_tokens=False)}{RESET}")
